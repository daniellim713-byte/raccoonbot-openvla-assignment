...
class SyncSimRaccoonDataset:
    ...
    def make_pick_and_place_plan(self, box_x, box_y, target_color):
        z_above = 0.10
        z_grasp = 0.02
        z_carry = 0.08
        pad_x, pad_y = self.PAD_POSITIONS[target_color]
        return [
            [box_x, box_y, z_above, 0],
            [box_x, box_y, z_grasp, 0],
            [box_x, box_y, z_grasp, 1],
            [box_x, box_y, z_carry, 1],
            [pad_x, pad_y, z_carry, 1],
            [pad_x, pad_y, z_grasp, 1],
            [pad_x, pad_y, z_grasp, 0],
        ]
...
def run_episode_and_record(
    ...
    try:
        ...
        plan = rc.make_pick_and_place_plan(target_x, target_y, target_color)
        ...
        # pick-and-place success: 오브젝트가 패드 근처에 있는지 확인
        body_id = mujoco.mj_name2id(rc.model, mujoco.mjtObj.mjOBJ_BODY, target_body_name)
        pad_x, pad_y = rc.PAD_POSITIONS[target_color]
        obj_x = float(rc.data.xpos[body_id][0])
        obj_y = float(rc.data.xpos[body_id][1])
        import math
        dist = math.sqrt((obj_x - pad_x)**2 + (obj_y - pad_y)**2)
        success = dist < 0.08
...
def collect_dataset(
    ...
    dataset_root="raccoon_recycling_colored",
    num_episodes=100,
